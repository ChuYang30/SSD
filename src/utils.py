...
class Encoder(object):
    ...
    def encode(self, bboxes_in, labels_in, criteria=0.5):   #传入gtbox的相关信息
        """
        encode:
            input  : bboxes_in (Tensor nboxes x 4), labels_in (Tensor nboxes)
            output : bboxes_out (Tensor 8732 x 4), labels_out (Tensor 8732)
        ...
        # [8732,]
        best_dbox_ious, best_dbox_idx = ious.max(dim=0)  # 寻找每个default box匹配到的最大IoU及对应gtbox的索引
        # [nboxes,]
        best_bbox_ious, best_bbox_idx = ious.max(dim=1)  # 寻找每个GT匹配到的最大IoU及对应default box的索引
...
class DefaultBoxes(object):   #用于生成defaultbox
    def __init__(self, fig_size, feat_size, steps, scales, aspect_ratios, scale_xy=0.1, scale_wh=0.2):
        self.fig_size = fig_size   # 输入网络的图像大小 300（以SSD300为例）
        # [38, 19, 10, 5, 3, 1]，列表中的数据对应每个预测特征层的尺寸
        self.feat_size = feat_size  # 每个预测层的feature map尺寸（一共有6个预测特征层，大小各不相同）
        ...
        # [21, 45, 99, 153, 207, 261, 315]，列表中的数值为相对应的尺度
        self.scales = scales  # 每个特征层上预测的default box的scale（尺度）

        fk = fig_size / np.array(steps)     # 计算每层特征层的fk
        # [[2], [2, 3], [2, 3], [2, 3], [2], [2]]
        self.aspect_ratios = aspect_ratios  # 每个预测特征层上预测的default box的ratios

        self.default_boxes = []   #存储生成的每一个defaultbox坐标信息
        # size of feature and number of feature
        # 遍历每层特征层，计算default box
        for idx, sfeat in enumerate(self.feat_size):   #feat_size为每一个预测特征层的尺寸
            sk1 = scales[idx] / fig_size  # scale转为相对值[0-1]，fig_size为输入图像尺寸300
            sk2 = scales[idx + 1] / fig_size  # scale转为相对值[0-1]，使用本层和下一层的尺度
            sk3 = sqrt(sk1 * sk2)   #根号下sk1×sk2
            ...
            # 计算当前特征层对应原图上的所有default box（将default box放入原图中的对应位置）
            for w, h in all_sizes:   #遍历每一个default box的高度和宽度
                for i, j in itertools.product(range(sfeat), repeat=2):  # i -> 行（y）， j -> 列（x），将default box生成到对应的每个位置上，i对应行信息，j对应列信息
                    # 计算每个default box的中心坐标（范围是在0-1之间）   default box的坐标为相对坐标，可以理解成在特征图上生成，也可以理解成在原图上生成
                    cx, cy = (j + 0.5) / fk[idx], (i + 0.5) / fk[idx]  #x方向对应列，y方向对应行
                    self.default_boxes.append((cx, cy, w, h))

        # 将default_boxes转为tensor格式，此处default box的展示格式为中心坐标、高度和宽度
        self.dboxes = torch.as_tensor(self.default_boxes, dtype=torch.float32)  # 这里不转类型会报错
        self.dboxes.clamp_(min=0, max=1)  # 将坐标（x, y, w, h）都限制在0-1之间，裁剪操作

        # For IoU calculation
        # ltrb is left top coordinate and right bottom coordinate
        # 将(x, y, w, h)转换成(xmin, ymin, xmax, ymax)，方便后续计算IoU(匹配正负样本时)
        self.dboxes_ltrb = self.dboxes.clone()  #此处default box的展示格式为左上角坐标和右下角坐标
        self.dboxes_ltrb[:, 0] = self.dboxes[:, 0] - 0.5 * self.dboxes[:, 2]   # xmin  左上角x坐标
        self.dboxes_ltrb[:, 1] = self.dboxes[:, 1] - 0.5 * self.dboxes[:, 3]   # ymin  左上角y坐标
        self.dboxes_ltrb[:, 2] = self.dboxes[:, 0] + 0.5 * self.dboxes[:, 2]   # xmax  右下角x坐标
        self.dboxes_ltrb[:, 3] = self.dboxes[:, 1] + 0.5 * self.dboxes[:, 3]   # ymax  右下角y坐标
...
class PostProcess(nn.Module):  #后处理
    def __init__(self, dboxes):
        ...
        self.criteria = 0.5   #IOU阈值（非极大值处理时使用）
        self.max_output = 100  #最多只输出100个目标，对输出进行限制
    ...
    def scale_back_batch(self, bboxes_in, scores_in):
        ...
        bboxes_in = bboxes_in.permute(0, 2, 1)   #调整维度
        # [batch, label_num, 8732] -> [batch, 8732, label_num]   label_num为所需检测的类别个数+背景个数
        scores_in = scores_in.permute(0, 2, 1)
        # print(bboxes_in.is_contiguous())
    ...
    def decode_single_new(self, bboxes_in, scores_in, criteria, num_output):
        ...
        """
        device = bboxes_in.device
        num_classes = scores_in.shape[-1]   #类别个数
        ...
        # remove prediction with the background label
        # 移除归为背景类别的概率信息
        bboxes_in = bboxes_in[:, 1:, :]  # [8732, 21, 4] -> [8732, 20, 4]，0对应的为背景，所以从索引为1的地方开始取值
        scores_in = scores_in[:, 1:]  # [8732, 21] -> [8732, 20]
        labels = labels[:, 1:]  # [8732, 21] -> [8732, 20]

        # batch everything, by making every class prediction be a separate instance
        bboxes_in = bboxes_in.reshape(-1, 4)  # [8732, 20, 4] -> [8732x20, 4]   对数据进行展开
        ...
        inds = torch.where(torch.gt(scores_in, 0.05))[0]    #获取概率大于0.05的索引
        bboxes_in, scores_in, labels = bboxes_in[inds, :], scores_in[inds], labels[inds]   #获取在相应索引上的数值

        # remove empty boxes   移除面积很小的default box
        ws, hs = bboxes_in[:, 2] - bboxes_in[:, 0], bboxes_in[:, 3] - bboxes_in[:, 1]
        ...
        # non-maximum suppression  利用非极大值抑制算法进行重叠目标的过滤
        keep = batched_nms(bboxes_in, scores_in, labels, iou_threshold=criteria)
    ...
    def forward(self, bboxes_in, scores_in):   #预测每个default box的坐标偏移量和类别
